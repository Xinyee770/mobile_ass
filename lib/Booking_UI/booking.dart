...
class _BookingPageState extends State<BookingPage> {
  ...
  Future<void> _fetchBusySlots() async {
    ...
    try {
      ...
      final courseData = await supabase.from('courses').select('course_start').eq('instructor_id', selectedInstructorId).eq('date', dateStr);

      setState(() {
        final privateBusy = (bookingData as List).map((e) => e['start_time'].toString());
        final publicBusy = (courseData as List).map((e) => e['course_start']?.toString() ?? "");
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: const Text("Private Lesson", style: TextStyle(color: Colors.white)), backgroundColor: Colors.transparent),
    ...
    return DropdownButtonFormField<dynamic>(
      ...
      onChanged: (val) {
        setState(() {
          ...
          displayedInstructors = course['instructor'] != null ? [course['instructor']] : [];
        });
      },
    );
  }
